...
class BloomFilter(object):
        ...
        def __init__(self, size, number_expected_elemets=100000):
                self.size = size
                self.number_expected_elements = number_expected_elemets
...
for i in range(1, 100000):
        if not bloom_filter.check_is_not_in_filter(base_ip + str(1)):
                print(base_ip+str(1))